...
const STATUS_STYLES = {
  accepted:     { bg: "bg-green-900/40",  border: "border-green-500/40",  text: "text-green-300",  icon: "check" },
  rejected:     { bg: "bg-red-900/40",    border: "border-red-500/40",    text: "text-red-300",    icon: "x" },
  interview:    { bg: "bg-blue-900/40",   border: "border-blue-500/40",   text: "text-blue-300",   icon: "calendar" },
  missing_docs: { bg: "bg-yellow-900/40", border: "border-yellow-500/40", text: "text-yellow-300", icon: "clock" },
  info:         { bg: "bg-[var(--surface-hover)]/40",  border: "border-[var(--border-strong)]/40",  text: "text-[var(--ink-dim)]",  icon: "mail" },
};
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-2xl font-extrabold text-[var(--ink)] mb-2">{t("email.linkTitle")}</h1>
        <p className="text-[var(--ink-faint)] text-sm leading-relaxed">{t("email.linkDesc")}</p>
      </div>
      ...
      {/* How it works */}
      <div className="bg-[var(--surface-hover)]/60 border border-[var(--border)]/40 rounded-2xl p-5 mb-6">
        <p className="text-[var(--ink-dim)] font-semibold text-sm mb-3">{t("email.howItWorks")}</p>
        <ol className="text-[var(--ink-faint)] text-xs space-y-2">
      ...
      {/* Email input */}
      <div className="mb-4">
        <label className="block text-[var(--ink-dim)] text-sm font-semibold mb-1.5">{t("email.yourEmail")}</label>
        <input
          type="email"
          value={email}
          onChange={e => setEmail(e.target.value)}
          placeholder="you@gmail.com"
          className="w-full bg-[var(--surface-hover)] border border-[var(--border-strong)] rounded-xl px-4 py-3 text-[var(--ink)] text-sm placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition"
        />
      </div>
      ...
      <label className="flex items-start gap-3 cursor-pointer mb-6">
        ...
        />
        <span className="text-[var(--ink-faint)] text-xs leading-relaxed">{t("email.consentText")}</span>
      </label>

      <button
        onClick={submit}
        disabled={!email || !agreed || loading}
        className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-indigo-600 to-blue-600 text-[var(--ink)] font-bold text-sm hover:opacity-90 transition disabled:opacity-40 disabled:cursor-not-allowed"
      >
  ...
  return (
    <div className="max-w-lg mx-auto">
      <div className="text-center mb-6">
        <div className="mb-3 flex justify-center"><Icon d={ICONS.settings} size={32} /></div>
        <h2 className="text-xl font-extrabold text-[var(--ink)] mb-1">{t("email.setupTitle")}</h2>
      ...
      <div className="bg-indigo-900/30 border border-indigo-600/40 rounded-2xl px-5 py-4 mb-6 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[var(--ink)] font-mono font-bold text-base">{FORWARDING_ADDRESS}</p>
        </div>
        <button
          onClick={() => { navigator.clipboard.writeText(FORWARDING_ADDRESS); toast.success(t("email.copied")); }}
          className="text-xs bg-indigo-600/50 hover:bg-indigo-600 text-[var(--ink)] px-3 py-1.5 rounded-lg transition font-semibold shrink-0"
        >
          {t("email.copy")}
        </button>
      </div>
      ...
      <div className="flex gap-2 mb-5">
        {["gmail", "outlook"].map(provider => (
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-semibold transition ${tab === provider ? "bg-indigo-600 text-[var(--ink)]" : "bg-[var(--surface-hover)] text-[var(--ink-faint)] hover:text-[var(--ink)]"}`}
          >
      ...
      <div className="space-y-3 mb-6">
        {steps.map(s => (
          <div key={s.step} className="flex gap-4 bg-[var(--surface-hover)]/50 border border-[var(--border)]/40 rounded-2xl px-4 py-3">
            ...
            <div>
              <p className="text-[var(--ink)] text-sm font-semibold">{s.title}</p>
              <p className="text-[var(--ink-faint)] text-xs mt-0.5">{s.desc}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <button
        onClick={confirm}
        disabled={confirming}
        className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-green-600 to-emerald-600 text-[var(--ink)] font-bold text-sm hover:opacity-90 transition mb-3"
      >
        {confirming ? t("email.confirming") : t("email.activateBtn")}
      </button>
      <button
        onClick={onUnlink}
        className="w-full py-2.5 rounded-2xl border border-[var(--border)] text-[var(--ink-faint)] hover:text-[var(--ink)] hover:border-slate-500 text-sm font-medium transition"
      >
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-4 bg-green-900/20 border border-green-600/30 rounded-2xl px-5 py-4 mb-6">
        ...
        <button
          onClick={onRefresh}
          className="text-xs text-[var(--ink-faint)] hover:text-[var(--ink)] transition px-3 py-1.5 bg-[var(--surface-hover)] rounded-lg"
        >
          {t("email.refresh")}
        </button>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          { icon: ICONS.mail, label: t("email.emailArrives"),   desc: t("email.emailArrivesDesc") },
          { icon: ICONS.search, label: t("email.weDetectIt"),     desc: t("email.weDetectItDesc") },
          { icon: ICONS.notifications, label: t("email.youreNotified"),  desc: t("email.youreNotifiedDesc") },
        ].map(item => (
          <div key={item.label} className="bg-[var(--surface-hover)]/50 border border-[var(--border)]/40 rounded-2xl p-3 text-center">
            <div className="mb-1 flex justify-center"><Icon d={ICONS[item.icon]} size={20} /></div>
            <p className="text-[var(--ink)] text-xs font-semibold">{item.label}</p>
            <p className="text-[var(--ink-faint)] text-xs mt-0.5">{item.desc}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-[var(--ink)] font-bold text-base">{t("email.universityEmails")}</h3>
        <span className="text-[var(--ink-faint)] text-xs">{emails.length} {emails.length !== 1 ? t("email.emailCount_other", { count: emails.length }) : t("email.emailCount_one", { count: emails.length })}</span>
      </div>

      {emails.length === 0 ? (
        <div className="text-center py-12 text-[var(--ink-faint)]">
          <div className="mb-3 flex justify-center"><Icon d={ICONS.mail} size={32} /></div>
        ...
        <div className="space-y-2">
          {emails.map(email => {
            ...
            return (
              ...
              >
                <span className="shrink-0 mt-0.5"><Icon d={ICONS[style.icon]} size={18} /></span>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                    {email.detected_university && (
                      <span className="text-[var(--ink)] font-semibold text-sm">{email.detected_university}</span>
                    )}
                  ...
                  <p className="text-[var(--ink-dim)] text-xs truncate">{email.subject}</p>
      ...
      <div className="mt-8 pt-6 border-t border-[var(--border)]">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[var(--ink)] font-semibold text-sm">{t("email.removeTitle")}</p>
            <p className="text-[var(--ink-faint)] text-xs mt-0.5">{t("email.removeDesc")}</p>
          </div>
          <button
            onClick={onUnlink}
