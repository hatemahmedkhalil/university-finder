...
const LANG_META = {
  english: { flag: "🇬🇧", code: "GB", color: "var(--accent)", next: { A1:"A2", A2:"B1", B1:"B2", B2:"C1", C1:"C2", C2:"C2", native:"native" } },
  german:  { flag: "🇩🇪", code: "DE", color: "var(--danger)",  next: { A1:"A2", A2:"B1", B1:"B2", B2:"C1", C1:"C2", C2:"C2", native:"native" } },
  polish:  { flag: "🇵🇱", code: "PL", color: "var(--good)", next: { A1:"A2", A2:"B1", B1:"B2", B2:"C1", C1:"C2", C2:"C2", native:"native" } },
};
  ...
  return (
    <div className="min-h-screen" style={{ background: BG }}>
      ...
      <div className="relative overflow-hidden" style={{ height: 240 }}>
        ...
        />
        <div className="absolute inset-0" style={{ background: "linear-gradient(to right, rgba(3,3,8,0.85), transparent)" }} />
        <div className="absolute inset-0 flex flex-col justify-end px-8 pb-8">
          <h1 className="text-3xl font-extrabold text-[var(--ink)]">Learning Center</h1>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-10">
        ...
        {hasAnyLang && (
          <section>
            <h2 className="text-[var(--ink)] font-bold text-lg mb-4">Your languages</h2>
                  ...
                  <div key={lang} className="rounded-2xl p-5 flex items-center gap-4"
                    ...
                    <div className="relative shrink-0">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span className="text-[var(--ink)] font-bold text-sm">{pct}%</span>
                      </div>
                    </div>
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5 mb-0.5">
                        ...
                        <span className="text-[var(--ink)] font-bold capitalize">{lang}</span>
                      </div>
        ...
        <div className="rounded-2xl px-6 py-5 flex items-center justify-between gap-4"
          ...
          <div>
            <p className="text-[var(--ink)] font-bold">Not sure where to start?</p>
          ...
          <Link to="/learning/placement/english"
            className="shrink-0 text-[var(--ink)] text-sm font-bold px-5 py-2.5 rounded-xl transition hover:opacity-90 whitespace-nowrap"
        ...
        {/* ── Courses ── */}
        <section>
          <h2 className="text-[var(--ink)] font-bold text-lg mb-4">Courses</h2>
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[1,2,3].map(i => (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {Object.entries(LANG_META).map(([lang, { flag, code, color }]) => (
                <Link key={lang} to={`/learning/courses/${lang}`}
                  ...
                  style={{ background: SURF, border: `1px solid ${BORDER}` }}>
                  <div className="h-28 flex items-center justify-center text-3xl font-black text-[var(--ink)]"
                    style={{ background: color, opacity: 0.85 }}>
                    {code}
                  </div>
                  <div className="p-4">
                    <p className="text-[var(--ink)] font-bold capitalize">{lang === "english" ? "English" : lang === "german" ? "German" : "Polish"} Courses</p>
              ...
              {courses.filter(c => c.is_published).slice(0, 6).map(course => {
                ...
                return (
                  <Link key={course.id} to={`/learning/courses/${course.language}/${course.id}`}
                    ...
                    <div className="p-4">
                      <p className="text-[var(--ink)] font-bold text-sm leading-snug">{course.title}</p>
        ...
        {/* ── Placement Tests ── */}
        <section>
          <h2 className="text-[var(--ink)] font-bold text-lg mb-4">Placement Tests</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {Object.entries(LANG_META).map(([lang, { flag, code, color }]) => (
              <Link key={lang} to={`/learning/placement/${lang}`}
                ...
                style={{ background: SURF, border: `1px solid ${BORDER}` }}>
                <div className="w-12 h-12 rounded-xl flex items-center justify-center text-lg font-black text-[var(--ink)] shrink-0"
                  style={{ background: color }}>
                  {code}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[var(--ink)] font-bold text-sm capitalize">
        ...
        {/* ── IELTS Simulator ── */}
        <section>
          <h2 className="text-[var(--ink)] font-bold text-lg mb-4">IELTS Simulator</h2>
          ...
          <Link to="/learning/ielts"
            ...
            <div className="flex gap-2 shrink-0">
              {[ICONS.headphones, ICONS.book, ICONS.pencil, ICONS.mic].map((icon, i) => (
                <div key={i} className="w-10 h-10 rounded-xl flex items-center justify-center text-white"
            ...
            <div className="flex-1">
              <p className="text-[var(--ink)] font-bold">IELTS Full Practice Test</p>
