  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-2xl mx-auto px-6 py-8 space-y-4">
        ...
        {/* Account info */}
        {user && (
          <Section icon={<Icon d={ICONS.profile} size={17} />} title={t("settings.account.title")} subtitle="Your account details">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-center gap-4 min-w-0">
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-[var(--ink)] text-xl font-bold shrink-0">
                  {user.email?.[0]?.toUpperCase()}
                </div>
                <div className="min-w-0">
                  <p className="font-semibold text-[var(--ink)] text-sm truncate">{user.email}</p>
                  <p className="text-[var(--ink-dim)] text-xs mt-0.5">{t("settings.account.email")}</p>
                </div>
              </div>
        ...
        {/* Language */}
        <Section icon={<Icon d={ICONS.globe} size={17} />} title={t("settings.language.title")} subtitle={t("settings.language.subtitle")}>
              ...
              <button key={code} onClick={() => handleLangChange(code)}
                className={`flex items-center gap-3 px-4 py-4 rounded-xl border-2 text-left transition-all ${
                  currentLang === code
                    ...
                    : "border-[rgba(255,255,255,0.08)] hover:border-indigo-500/50 hover:bg-[var(--surface-hover)]"
                }`}>
                <span className="text-2xl">{flag}</span>
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-[var(--ink)] text-sm">{name}</p>
                  <p className="text-[var(--ink-dim)] text-xs">{sub}</p>
                </div>
                {currentLang === code && (
                  <div className="w-5 h-5 rounded-full bg-indigo-500 flex items-center justify-center shrink-0">
                    <svg className="w-3 h-3 text-[var(--ink)]" fill="currentColor" viewBox="0 0 20 20">
        ...
        {/* Welcome tour */}
        <Section icon={<Icon d={ICONS.graduationCap} size={17} />} title={t("settings.onboarding.title")} subtitle={t("settings.onboarding.restartDesc")}>
          <button onClick={restartOnboarding}
            className="flex items-center gap-2 px-5 py-3 bg-indigo-600 text-[var(--ink)] rounded-xl font-semibold text-sm hover:bg-indigo-700 transition">
            {t("settings.onboarding.restart")}
          </button>
        </Section>

        {/* Danger zone */}
        <div className="bg-[var(--surface-2)] rounded-2xl border-2 border-red-500/30 overflow-hidden">
          <div className="flex items-center gap-3 px-6 py-4 border-b border-red-500/20 bg-red-500/10">
            <div className="w-9 h-9 rounded-xl bg-red-500/20 flex items-center justify-center"><Icon d={ICONS.alertTriangle} size={16} /></div>
            <div>
          ...
          <div className="p-6">
            <button onClick={logout}
              className="flex items-center gap-2 px-5 py-3 bg-red-500 text-[var(--ink)] rounded-xl font-bold text-sm hover:bg-red-600 transition  shadow-red-200">
